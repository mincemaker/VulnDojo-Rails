...
| Rails | 8.1.3 |
| DB | SQLite3 |
| キャッシュ/セッション | Redis 7 (`redis_cache_store`) |
| サーバー | Puma |
...
`compose.yaml` には `web`（Rails）と `redis` の2サービスが定義されています。Redis は SSRF チャレンジのセッションストアとして使用されます。
...
| 手法 | 用途 | 使用チャレンジ |
|------|------|------|
| `Module#prepend` | コントローラのアクションメソッドを上書き | xss, sqli, idor, open_redirect, mass_assign, session_fix, cmd_inj, broken_auth_timing, ssrf, xxe_nokogiri |
| `Base#inject_view` | ビューパーシャルを脆弱版に差し替え（`prepend_view_path` をラップ） | xss_raw, css_injection, xss_reflected, ssrf |
| `Base#add_routes` | ルートを動的追加（`routes.prepend` + `routes.draw`） | ssrf |
...
| `config` 操作 | Rails 設定を変更 | log_leakage, csp_disable, ssrf |
...
| `ssrf` | SSRF via URL preview (curl + gopher) | SSRF | Medium | CWE-918 |
| `xxe_nokogiri` | XXE via Nokogiri XML import | Injection | Hard | CWE-611 |
...
| ssrf | preview_url ルートが存在しない (404) | gopher→Redis でセッションデータをリーク / 内部 HTTP URL にアクセス可 |
| xxe_nokogiri | 正常XMLインポート成功、XXEペイロードが展開されない | `config.noent` で `/etc/passwd` の内容がタスクの description に漏洩 |
...
│   ├── engine.rb                        # ブートローダー
│   ├── challenges/                      # 21個のチャレンジ実装
│   │   ├── xss_raw.rb                   xss_reflected.rb
│   │   ├── xss_stored_img.rb            css_injection.rb
...
│   │   ├── csp_disable.rb               command_injection.rb
│   │   ├── broken_auth_timing.rb        ssrf.rb
│   │   ├── xxe_nokogiri.rb
│   │   └── (21 files)
│   └── views/tasks/                     # 脆弱なテンプレート（注入用）
├── test/integration/vulnerabilities/     # E2E テスト (110テスト, 264アサーション)
